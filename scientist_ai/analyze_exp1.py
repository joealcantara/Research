#!/usr/bin/env python3
"""Analyze Experiment 1 results: beam search vs exhaustive ground truth"""

import json
import glob
from pathlib import Path

# Optimal score from exhaustive search
OPTIMAL_SCORE = -412.960

# Load all run results
results = []
for i in range(1, 21):
    path = f"results/exp1/run_{i}.json"
    try:
        with open(path) as f:
            data = json.load(f)
...
print("=" * 60)
print("EXPERIMENT 1: GROUND TRUTH VALIDATION")
print("=" * 60)
print(f"Dataset: iris_4var (4 continuous variables, 150 samples)")
print(f"Exhaustive optimal: {OPTIMAL_SCORE:.3f}")
